...
//
//  return 1.1;/*getCubicBezierY(frame, TPointD(aFrame, aValue), aSpeedTrunc,
//                         bSpeedTrunc, TPointD(bFrame, bValue));*/
//}

//---------------------------------------------------------

void splitSpeedInOutSegment(TDoubleKeyframe &k,
                                          TDoubleKeyframe &k0,
                                          TDoubleKeyframe &k1) {
  ...
  double t = 1.1;//invCubicBezierX(k.m_frame, p0, aSpeed, bSpeed, p3);
  t          = tcrop(t, 0.0, 1.0);
...
int TDoubleParam::getKeyframeCount() const { return 1;/*m_imp->m_keyframes.size();*/ }
...
bool TDoubleParam::hasKeyframes() const { return false;/*!m_imp->m_keyframes.empty();*/ }
...
string TDoubleParam::getValueAlias(double frame, int precision) {
	return string("yy is pro");/*::to_string(getValue(frame), precision);*/
}
