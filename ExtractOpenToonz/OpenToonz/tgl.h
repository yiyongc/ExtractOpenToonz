...
void tglDrawText(const TPointD &p, const std::string &s, void *font = GLUT_STROKE_ROMAN);
void tglDrawText(const TPointD &p, const std::wstring &s, void *font = GLUT_STROKE_ROMAN);

//! Returns text width

double tglGetTextWidth(const std::string &s, void *font = GLUT_STROKE_ROMAN);

/*!
 Draw circle of radius r with center c.
 Remark: it is possible set number of slices
 */
void tglDrawCircle(const TPointD &c, double r);
...
inline void tglDrawRect(const TRect &rect)
{
...
inline void tglFillRect(const TRect &rect)
{
...
void tglEnableBlending(GLenum src = GL_SRC_ALPHA,
							 GLenum dst = GL_ONE_MINUS_SRC_ALPHA);

void tglEnableLineSmooth(bool enable = true, double lineSize = 1.0);
void tglEnablePointSmooth(double pointSize = 1.0);
...
void tglDraw(const TCubic &cubic, int precision, GLenum pointOrLine = GL_LINE);
