...
double buildAngle(const PlasticSkeleton &skeleton, int v) {
  ...
  return tcg::point_ops::angle(dir, vx.P() - vxParent.P()) * 180 / 3.14159;
}
...
void PlasticSkeletonDeformation::Imp::updateBranchPositions(
  ...
  if (vParent >= 0) {
	...
	double a = tcg::point_ops::angle(oDir, ovxPos - ovxParentPos) * 180 / 3.14159;
  ...
  struct locals {
    ...
    static void adjust(SkD &sd, int v) {
      ...
      if (vParent >= 0) {
		...
		double a = tcg::point_ops::angle(dir, vxPos - vxParentPos) * 180 / 3.14159;
